...
<#
.SYNOPSIS
Get-Exports, fetches the exports of all DLL under %windir%/system32. 
To do this it reads DLL bytes into memory and then parses them (no LoadLibraryEx).
Only support x32 DLL

.DESCRIPTION
Steal from https://github.com/FuzzySecurity/PowerShell-Suite/blob/master/Get-Exports.ps1
...
foreach($Export in $ExportArray)
{
    If($Export.FunctionName -eq "OpenURL")
    {
        Write-Host "[!]" $DllPath
    }
}  
...
{   
    $DllFullName = "c:\windows\system32\"+$DllName.Name
 #   Write-Host $DllFullName
    Get-Exports -DllPath $DllFullName
}
